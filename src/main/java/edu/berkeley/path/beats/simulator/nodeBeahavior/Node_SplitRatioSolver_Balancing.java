...
import edu.berkeley.path.beats.simulator.output.InterfaceOutputWriter;
...
public class Node_SplitRatioSolver_Balancing extends Node_SplitRatioSolver{
	...
	public Double [][][] computeAppliedSplitRatio(final Double [][][] splitratio_selected,final int e) {
		...
		assignZeroDemandSplits(splitratio_selected);
	...
	private void initializeAssignedSplits(final Double[][][] splitratio_selected) {
		for(int i=0;i<myNode.nIn;i++) {
			for(int c=0;c<nVType;c++) {
				...
				for(int j=0;j<myNode.nOut;j++) {
					...
					if (Double.isNaN(splitratio_selected[i][j][c])) {
						splitKnown[i][j][c] = false;
					}
					else {
						splitKnown[i][j][c] = true;
						splitRemaining[i][c] = splitRemaining[i][c] - splitratio_selected[i][j][c];
					}
				}
			}
		}
